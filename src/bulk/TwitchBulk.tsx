import React from 'react';
import { IconProps } from '../IconWrapper';

export const TwitchBulk: React.FC<IconProps> = ({
  size = 24,
  color = 'currentColor',
  ...props
}) => {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path opacity="0.4" fillRule="evenodd" clipRule="evenodd" d="M5.04001 2H20.5C21.05 2 21.5 2.45 21.5 3V13.59C21.5 13.86 21.39 14.11 21.21 14.3L16.8 18.71C16.61 18.9 16.36 19 16.09 19H12.04C11.71 19 11.39 19.17 11.21 19.45L9.80002 21.56C9.61002 21.84 9.30002 22.01 8.97002 22.01H7.51001C6.96001 22.01 6.51001 21.56 6.51001 21.01V20.01C6.51001 19.46 6.06001 19.01 5.51001 19.01H3.51001C2.96001 19.01 2.51001 18.56 2.51001 18.01V5.31C2.51001 5.11 2.57001 4.91999 2.68001 4.75999L4.22002 2.46001C4.39002 2.17001 4.70001 2 5.04001 2Z" fill="currentColor"/>
<path fillRule="evenodd" clipRule="evenodd" d="M8.5 7.1403V12.8503C8.5 13.1703 8.76001 13.4203 9.07001 13.4203H10.21C10.53 13.4203 10.78 13.1603 10.78 12.8503V7.1403C10.78 6.8203 10.52 6.57031 10.21 6.57031H9.07001C8.76001 6.57031 8.5 6.8303 8.5 7.1403Z" fill="currentColor"/>
<path fillRule="evenodd" clipRule="evenodd" d="M14.21 7.1403V12.8503C14.21 13.1703 14.47 13.4203 14.78 13.4203H15.92C16.24 13.4203 16.49 13.1603 16.49 12.8503V7.1403C16.49 6.8203 16.23 6.57031 15.92 6.57031H14.78C14.47 6.57031 14.21 6.8303 14.21 7.1403Z" fill="currentColor"/>
    </svg>
  );
};
