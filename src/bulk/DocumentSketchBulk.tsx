import React from 'react';
import { IconProps } from '../IconWrapper';

export const DocumentSketchBulk: React.FC<IconProps> = ({
  size = 24,
  color,
  style,
  ...props
}) => {
  const s = { ...(style || {}) } as Record<string, unknown>;
  const fromStyle = s.color;
  delete s.color;
  const resolvedColor: string =
    color !== undefined
      ? String(color)
      : fromStyle !== undefined && fromStyle !== null
        ? String(fromStyle)
        : 'currentColor';
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      style={{ ...(s as React.CSSProperties), color: resolvedColor }}
      {...props}
    >
      <path opacity="0.4" d="M20.5 10.19H17.61C15.24 10.19 13.31 8.26 13.31 5.89V3C13.31 2.45 12.86 2 12.31 2H8.07C4.99 2 2.5 4 2.5 7.57V16.43C2.5 20 4.99 22 8.07 22H15.93C19.01 22 21.5 20 21.5 16.43V11.19C21.5 10.64 21.05 10.19 20.5 10.19Z" fill="currentColor"/>
<path d="M15.8002 2.20999C15.3902 1.79999 14.6802 2.07999 14.6802 2.64999V6.13999C14.6802 7.59999 15.9202 8.80999 17.4302 8.80999C18.3802 8.81999 19.7002 8.81999 20.8302 8.81999C21.4002 8.81999 21.7002 8.14999 21.3002 7.74999C19.8602 6.29999 17.2802 3.68999 15.8002 2.20999Z" fill="currentColor"/>
<path d="M12.5501 13.9999L11.7001 12.7199C11.4901 12.3899 11.0501 12.1599 10.6601 12.1599H8.64011C8.25011 12.1599 7.82011 12.3899 7.60011 12.7199L6.75011 13.9999C6.42011 14.4799 6.49011 15.1699 6.90011 15.5799L8.76011 17.4399C9.00011 17.6699 9.32011 17.7999 9.65011 17.7999C9.98011 17.7999 10.3001 17.6699 10.5401 17.4399L12.4001 15.5799C12.8001 15.1699 12.8601 14.4799 12.5501 13.9999Z" fill="currentColor"/>
    </svg>
  );
};
