import React from 'react';
import { IconProps } from '../IconWrapper';

export const Grid8Bold: React.FC<IconProps> = ({
  size = 24,
  color = 'currentColor',
  ...props
}) => {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path d="M22 7.80978V11.2498H16.75V2.02979C20.05 2.22979 22 4.35978 22 7.80978Z" fill="currentColor"/>
<path d="M22 12.75V16.19C22 19.64 20.05 21.77 16.75 21.97V12.75H22Z" fill="currentColor"/>
<path d="M7.25 12.75V21.97C3.95 21.77 2 19.64 2 16.19V12.75H7.25Z" fill="currentColor"/>
<path d="M7.25 2.02979V11.2498H2V7.80978C2 4.35978 3.95 2.22979 7.25 2.02979Z" fill="currentColor"/>
<path d="M15.25 2H8.75V11.25H15.25V2Z" fill="currentColor"/>
<path d="M15.25 12.75H8.75V22H15.25V12.75Z" fill="currentColor"/>
    </svg>
  );
};
