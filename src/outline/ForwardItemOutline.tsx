import React from 'react';
import { IconProps } from '../IconWrapper';

export const ForwardItemOutline: React.FC<IconProps> = ({
  size = 24,
  color,
  style,
  ...props
}) => {
  const s = { ...(style || {}) } as Record<string, unknown>;
  const fromStyle = s.color;
  delete s.color;
  const resolvedColor: string =
    color !== undefined
      ? String(color)
      : fromStyle !== undefined && fromStyle !== null
        ? String(fromStyle)
        : 'currentColor';
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      style={{ ...(s as React.CSSProperties), color: resolvedColor }}
      {...props}
    >
      <path d="M13.6201 18.13H10.3901C7.26012 18.13 5.87012 16.74 5.87012 13.61V10.38C5.87012 7.25 7.26012 5.85999 10.3901 5.85999H13.6201C16.7501 5.85999 18.1401 7.25 18.1401 10.38V13.61C18.1301 16.74 16.7401 18.13 13.6201 18.13ZM10.3801 7.37C8.09012 7.37 7.36012 8.10001 7.36012 10.39V13.62C7.36012 15.91 8.09012 16.64 10.3801 16.64H13.6101C15.9001 16.64 16.6301 15.91 16.6301 13.62V10.39C16.6301 8.10001 15.9001 7.37 13.6101 7.37H10.3801Z" fill="currentColor"/>
<path d="M18.2305 13.52H17.3805C16.9705 13.52 16.6305 13.18 16.6305 12.77V10.39C16.6305 8.10001 15.9005 7.37 13.6105 7.37H11.2305C10.8205 7.37 10.4805 7.02999 10.4805 6.62V5.76999C10.4805 2.63999 11.8705 1.25 15.0005 1.25H18.2305C21.3605 1.25 22.7505 2.63999 22.7505 5.76999V9C22.7505 12.13 21.3605 13.52 18.2305 13.52ZM18.1305 12.02H18.2305C20.5205 12.02 21.2505 11.29 21.2505 9V5.76999C21.2505 3.47999 20.5205 2.75 18.2305 2.75H15.0005C12.7105 2.75 11.9805 3.47999 11.9805 5.76999V5.87H13.6105C16.7405 5.87 18.1305 7.26001 18.1305 10.39V12.02Z" fill="currentColor"/>
<path d="M9 22.75H5.77C2.64 22.75 1.25 21.36 1.25 18.23V15C1.25 11.87 2.64 10.48 5.77 10.48H6.62C7.03 10.48 7.37 10.82 7.37 11.23V13.61C7.37 15.9 8.1 16.63 10.39 16.63H12.77C13.18 16.63 13.52 16.97 13.52 17.38V18.23C13.52 21.36 12.13 22.75 9 22.75ZM5.77 11.98C3.48 11.98 2.75 12.71 2.75 15V18.23C2.75 20.52 3.48 21.25 5.77 21.25H9C11.29 21.25 12.02 20.52 12.02 18.23V18.13H10.39C7.26 18.13 5.87 16.74 5.87 13.61V11.98H5.77Z" fill="currentColor"/>
    </svg>
  );
};
