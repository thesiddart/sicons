import React from 'react';
import { IconProps } from '../IconWrapper';

export const CardRemove1Outline: React.FC<IconProps> = ({
  size = 24,
  color,
  style,
  ...props
}) => {
  const s = { ...(style || {}) } as Record<string, unknown>;
  const fromStyle = s.color;
  delete s.color;
  const resolvedColor: string =
    color !== undefined
      ? String(color)
      : fromStyle !== undefined && fromStyle !== null
        ? String(fromStyle)
        : 'currentColor';
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      style={{ ...(s as React.CSSProperties), color: resolvedColor }}
      {...props}
    >
      <path d="M13.5 9.25H2C1.59 9.25 1.25 8.91 1.25 8.5C1.25 8.09 1.59 7.75 2 7.75H13.5C13.91 7.75 14.25 8.09 14.25 8.5C14.25 8.91 13.91 9.25 13.5 9.25Z" fill="currentColor"/>
<path d="M8 17.25H6C5.59 17.25 5.25 16.91 5.25 16.5C5.25 16.09 5.59 15.75 6 15.75H8C8.41 15.75 8.75 16.09 8.75 16.5C8.75 16.91 8.41 17.25 8 17.25Z" fill="currentColor"/>
<path d="M14.5 17.25H10.5C10.09 17.25 9.75 16.91 9.75 16.5C9.75 16.09 10.09 15.75 10.5 15.75H14.5C14.91 15.75 15.25 16.09 15.25 16.5C15.25 16.91 14.91 17.25 14.5 17.25Z" fill="currentColor"/>
<path d="M17.56 21.25H6.44C2.46 21.25 1.25 20.05 1.25 16.11V7.89C1.25 3.95 2.46 2.75 6.44 2.75H13.5C13.91 2.75 14.25 3.09 14.25 3.5C14.25 3.91 13.91 4.25 13.5 4.25H6.44C3.3 4.25 2.75 4.79 2.75 7.89V16.1C2.75 19.2 3.3 19.74 6.44 19.74H17.55C20.69 19.74 21.24 19.2 21.24 16.1V12.02C21.24 11.61 21.58 11.27 21.99 11.27C22.4 11.27 22.74 11.61 22.74 12.02V16.1C22.75 20.05 21.54 21.25 17.56 21.25Z" fill="currentColor"/>
<path d="M17.31 8.94C17.12 8.94 16.93 8.87 16.78 8.72C16.49 8.43 16.49 7.95 16.78 7.66L20.67 3.77C20.96 3.48 21.44 3.48 21.73 3.77C22.02 4.06 22.02 4.54 21.73 4.83L17.84 8.72C17.69 8.87 17.5 8.94 17.31 8.94Z" fill="currentColor"/>
<path d="M21.1902 8.94C21.0002 8.94 20.8102 8.87 20.6602 8.72L16.7702 4.83C16.4802 4.54 16.4802 4.06 16.7702 3.77C17.0602 3.48 17.5402 3.48 17.8302 3.77L21.7202 7.66C22.0102 7.95 22.0102 8.43 21.7202 8.72C21.5802 8.87 21.3902 8.94 21.1902 8.94Z" fill="currentColor"/>
    </svg>
  );
};
