import React from 'react';
import { IconProps } from '../IconWrapper';

export const AdditemOutline: React.FC<IconProps> = ({
  size = 24,
  color,
  style,
  ...props
}) => {
  const s = { ...(style || {}) } as Record<string, unknown>;
  const fromStyle = s.color;
  delete s.color;
  const resolvedColor: string =
    color !== undefined
      ? String(color)
      : fromStyle !== undefined && fromStyle !== null
        ? String(fromStyle)
        : 'currentColor';
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      style={{ ...(s as React.CSSProperties), color: resolvedColor }}
      {...props}
    >
      <path d="M8 16.75H5.43C2.74 16.75 1.25 15.27 1.25 12.57V5.43C1.25 2.74 2.73 1.25 5.43 1.25H10C12.69 1.25 14.18 2.73 14.18 5.43C14.18 5.84 13.84 6.18 13.43 6.18C13.02 6.18 12.68 5.84 12.68 5.43C12.68 3.55 11.88 2.75 10 2.75H5.43C3.55 2.75 2.75 3.55 2.75 5.43V12.57C2.75 14.45 3.55 15.25 5.43 15.25H8C8.41 15.25 8.75 15.59 8.75 16C8.75 16.41 8.41 16.75 8 16.75Z" fill="currentColor"/>
<path d="M18.5703 22.75H14.0003C11.3103 22.75 9.82031 21.27 9.82031 18.57V11.43C9.82031 8.74 11.3003 7.25 14.0003 7.25H18.5703C21.2603 7.25 22.7503 8.73 22.7503 11.43V18.57C22.7503 21.27 21.2703 22.75 18.5703 22.75ZM14.0003 8.75C12.1203 8.75 11.3203 9.55 11.3203 11.43V18.57C11.3203 20.45 12.1203 21.25 14.0003 21.25H18.5703C20.4503 21.25 21.2503 20.45 21.2503 18.57V11.43C21.2503 9.55 20.4503 8.75 18.5703 8.75H14.0003Z" fill="currentColor"/>
<path d="M18.1299 15.75H14.8799C14.4699 15.75 14.1299 15.41 14.1299 15C14.1299 14.59 14.4699 14.25 14.8799 14.25H18.1299C18.5399 14.25 18.8799 14.59 18.8799 15C18.8799 15.41 18.5399 15.75 18.1299 15.75Z" fill="currentColor"/>
<path d="M16.5 17.38C16.09 17.38 15.75 17.04 15.75 16.63V13.38C15.75 12.97 16.09 12.63 16.5 12.63C16.91 12.63 17.25 12.97 17.25 13.38V16.63C17.25 17.04 16.91 17.38 16.5 17.38Z" fill="currentColor"/>
    </svg>
  );
};
